package tir.ir.interpret;

import tir.util.InternalCompilerError;
import tir.ir.ast.*;
...
public class Simulator {
    ...
    public static final int DEFAULT_HEAP_SIZE = 1000000;
    ...
    protected void leave(ExecutionFrame frame) {
        ...
        if (insn instanceof Const)
        ...
        else if (insn instanceof GlobalVar) {
            String varName = ((GlobalVar) insn).name();
            exprStack.pushGlobalVar(globalVariables.getOrDefault(varName, 0), varName);
        }
        ...
        else if (insn instanceof CallStmt) {
            int argsCount = ((CallStmt) insn).args().size();
            int args[] = new int[argsCount];
            for (int i = argsCount - 1; i >= 0; --i)
                args[i] = exprStack.popValue();
            StackItem target = exprStack.pop();
            ...
            else throw new InternalCompilerError("Invalid function callstmt '"
                        + insn + "' (target '" + target.value + "' is unknown)!");
            ...
            case GLOBALVAR:
                if (debugLevel > 0)
                    System.out.println("globalVar[" + stackItem.name + "]=" + r);
                globalVariables.put(stackItem.name, r);
                break;
            default:
    ...
    private static class ExprStack {
        ...
        public void pushGlobalVar(int value, String name) {
            if (debugLevel > 1)
                System.out.println("Pushing GlobalVar " + value + " (" + name + ")");
            stack.push(new StackItem(StackItem.Kind.GLOBALVAR, value, name));
        }
    ...
    public static class StackItem {
        public enum Kind {
            COMPUTED, MEM, TEMP, NAME, GLOBALVAR
        }
        ...
        public StackItem(Kind type, int value, String string) {
            ...
            if (type == Kind.TEMP)
                temp = string;
            else if (type == Kind.NAME || type == Kind.GLOBALVAR)
                name = string;
            else
